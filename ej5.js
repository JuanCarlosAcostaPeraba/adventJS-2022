...
function getMaxGifts(giftsCities, maxGifts, maxCities) {
  const dfs = (giftsCities, maxGifts, maxCities, i = 0, countOfGifts = 0, currentCities = 0) => {
    if (currentCities == maxCities || i == giftsCities.length)
      return countOfGifts;

    if (countOfGifts + giftsCities[i] <= maxGifts)
      return Math.max(
        dfs(giftsCities, maxGifts, maxCities, i + 1, countOfGifts + giftsCities[i], currentCities + 1),
        dfs(giftsCities, maxGifts, maxCities, i + 1, countOfGifts, currentCities)
      );

    return dfs(giftsCities, maxGifts, maxCities, i + 1, countOfGifts, currentCities);
  }
  return dfs(giftsCities, maxGifts, maxCities);
}
